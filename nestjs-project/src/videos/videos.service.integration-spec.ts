...
describe('VideosService (integration)', () => {
  ...
  beforeAll(async () => {
    ...
    processingQueue = module.get<Queue>(getQueueToken(VIDEO_PROCESSING_QUEUE));
  }, 60000);
  ...
  it('completes a real two-part upload and enqueues exactly one processing job', async () => {
    const { userId } = await createOwner();
    const created = await service.createDraft(userId, DRAFT_INPUT);
    touchedKeys.push(`videos/${created.id}/`);

    const chunks = [Buffer.alloc(MIN_PART_SIZE, 'a'), Buffer.from('tail')];
    const presigned = await service.presignParts(userId, created.id, [1, 2]);
    const parts = await Promise.all(
      presigned.map(async (part, index) => {
        const response = await fetch(part.url, {
          method: 'PUT',
          body: chunks[index],
        });
    ...
    // Pausing keeps the job in the waiting set so the assertion cannot race a
    // worker that happens to be running against the same Redis.
    await processingQueue.pause();
    try {
      const result = await service.completeUpload(userId, created.id, {
        parts,
      });
      ...
      // The declared size was 20MiB; storage knows the real one.
      expect(stored.size_bytes).toBe(MIN_PART_SIZE + chunks[1].length);

      const waiting = await processingQueue.getWaiting();
      expect(waiting).toHaveLength(1);
      expect(waiting[0].id).toBe(created.id);
      expect(waiting[0].data).toEqual({ videoId: created.id });
